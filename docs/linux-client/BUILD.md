...
| `--input-script` | After StartGame, inject Crystal keys. `Drag` / `Drag:0,8` → `C.MoveItem` (`MirGridType.Inventory`); `Merge:0,1` → `C.MergeItem`. Map teleport stays `Move:x:y` (`@MOVE`). |
